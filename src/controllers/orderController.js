...
async function getOrder(req, res) {
  ...
  try {
    ...
    if (!order) {
      ...
      return res.status(404).send("?-?°???????»?µ?????? ???µ ?·???°?????µ????");
...
async function reserveOrder(req, res) {
  ...
  try {
    ...
    if (!order || order.status !== "CREATED") {
      ...
      return res.status(400).send("?-?°???????»?µ?????? ???µ???????,???????µ");
...
async function updateFinalPrice(req, res) {
  ...
  try {
    ...
    if (!order) return res.status(404).send("?-?°???????»?µ?????? ???µ ?·???°?????µ????");
    ...
    if (!Number.isFinite(n) || n <= 0) {
      ...
      return res.status(400).send("???µ???????µ???,???° ???????°");
...
async function respondToOrder(req, res) {
  ...
  try {
    ...
    if (activeCount >= MAX_ACTIVE_RESPONSES) {
      return res.status(400).send("?>?-???-?, ?°???,?????????. ???-?????????-?? (MAX). ?-?°???µ?????-?,?? ?????,???????- ???±???????????µ??????.");
    }
...
async function responseCallMade(req, res) {
  ...
  try {
    ...
    if (!order) return res.status(400).send("?-?°???????»?µ?????? ???µ ?·???°?????µ????");
